...
export const useCreateEditor = (
  {
    ...
    placeholders,
    readOnly,
    // onChange is not a valid property here, so it has been removed
    ...options
  }: {
  ...
  return usePlateEditor<Value, (typeof editorPlugins)[number]>(
    {
      ...
      // En lugar de usar initialContent que no existe, usamos value directamente
      value: options.value ? (typeof options.value === 'string' ? JSON.parse(options.value) : options.value) : [
        {
          children: [{ text: 'Comienza a escribir...' }],
          type: 'h1',
        },
        {
          children: [
            { text: 'Utiliza los comandos ' },
            { bold: true, text: 'AI' },
            { text: ' o presiona ' },
            { kbd: true, text: 'Cmd+J' },
            { text: ' para abrir el menú de AI.' },
          ],
          type: ParagraphPlugin.key,
        },
      ],
      // onChange ya fue pasado antes, eliminamos el duplicado
      ...options,
    },
    deps
  );
